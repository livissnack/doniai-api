...
class ArticleSchema extends Schema {
  up () {
    this.create('articles', (table) => {
      ...
      table.integer('comment_nums').notNullable().comment('文章评论总数')
      table.text('content').notNullable().comment('文章内容')
      table.string('image').comment('文章封面图片')
      table.integer('status').notNullable().comment('文章状态 0：上线，1：下线')
      table.integer('is_free').notNullable().comment('文章是否免费，0：免费，1：收费')
      table.integer('price').notNullable().comment('文字阅读价格-单位分')
